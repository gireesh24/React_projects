...
import {addTheatre,updateTheatres} from "../../api/theatre"
  ...
  const {user}=useSelector((state)=>state.users);
    ...
    const onFinish = async (values) => {
      try {
        ...
        let response = null;
        if (formType === "add") {
          response = await addTheatre({...values,ownerId:user._id});
        } else {
          values.theatreId=selectdTheatre._id
          response = await updateTheatres(values);
        }
        if (response.success) {
          getData();
